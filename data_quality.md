...
1) **В таблице users поле login должно быть уникальным, в настоящий момент соответствующее ограничение в БД отсутствует.**
   ...
   В данном случае, все логины уникальны, что проверяется запросом ниже:
...
3) **В таблице products price должна быть положительной.**
   ...
   Также для поля price создано ограничение products_price_cl типа CHECK, что не позволяет цене товара быть отрицательной или нулевой.
...
| production.orders | order_id int | Первичный ключ orders_pkey | Обеспечивает уникальность записей о заказа
